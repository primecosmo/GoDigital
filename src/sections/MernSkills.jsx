...
export default function MernSkills() {
  ...
  return (
    <section className="relative bg-[#0F0F0F] px-6 sm:px-16 py-32 overflow-hidden">
      
      {/* GREEN GLOW */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[600px] bg-[#22C55E]/10 blur-[180px] rounded-full" />
      ...
      <div className="max-w-5xl mx-auto grid gap-8 lg:grid-cols-2 relative z-10">
        {skills.map((skill, i) => (
          <motion.div
            ...
            className="flex flex-col gap-4 p-6 bg-[#171717] border border-[#2F2F2F] rounded-2xl shadow-[0_0_60px_rgba(34,197,94,0.2)] hover:shadow-[0_0_80px_rgba(34,197,94,0.4)] transition-all duration-500"
          >
            <div className="flex items-center gap-4">
              
              <div className="text-[#22C55E] text-3xl">
                {skill.icon}
              </div>
              ...
              <div className="ml-auto flex items-center gap-1 text-[#22C55E]">
                <FaCertificate />
            ...
            {/* Progress Bar */}
            <div className="w-full h-5 bg-[#2F2F2F] rounded-full overflow-hidden mt-2">
              <motion.div
                initial={{ width: 0 }}
                whileInView={{ width: `${skill.level}%` }}
                transition={{ duration: 1.5, ease: "easeOut" }}
                className="h-5 bg-[#22C55E] rounded-full shadow-[0_0_25px_rgba(34,197,94,0.5)]"
              />
            </div>
            ...
            <div className="mt-4 flex gap-4">
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                className="px-4 py-2 bg-[#22C55E] hover:bg-[#16A34A] text-black font-semibold rounded-xl transition-all duration-300"
              >
                Learn More
              </motion.button>
            </div>
      ...
      >
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="mt-6 px-8 py-3 bg-[#22C55E] hover:bg-[#16A34A] text-black font-bold rounded-2xl text-lg transition-all duration-300"
        >
          Get Started
        </motion.button>
      </motion.div>
